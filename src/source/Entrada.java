...
public class Entrada extends JFrame {
	...
	public Entrada() {
		configLKF("Nimbus");
		pDao = new ProfessorDao(); //inicializa gerenciador de daodos
	...
	private void configPaineis() {
		...
		for(Professor prof: pDao.getAll()) { //add nome dos profs
			profComboModel.addElement(prof.toString());
		}
		...
		professorEntrar.addActionListener( //classe anonima para entrar na area do prof
			new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					String profNome =  String.valueOf(profComboModel.getSelectedItem());
					int profId = Integer.parseInt(profNome.split(" ")[0]); //pega o id 

					new MateriaTela(pDao.get(profId));
				}
		}); 
		...
		professorCadastrar.addActionListener( //class anonima inicia frame de cadastro
			new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					
					if(cadFrame == null) { //se o frame nao estiver instanciado
						cadFrame = new CadastrarProfessorFrame();
	...
	class CadastrarProfessorFrame extends JFrame {
		...
		private void configPaineis() {
			...
			cadastrarPf.addActionListener( //classe anonima para cadastrar prof
				new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						...
						if(nomeVal && sbNomeVal) { //se ambos sao validos ( cadastra )
							...
							if(resp == JOptionPane.YES_OPTION) { //se resp sim, cadastra prof
								...
								pDao.save(prf);
								Entrada.profComboModel.addElement(prf.toString());
								
								JOptionPane.showMessageDialog(
									null, String.format("Professor(a) \'%s %s\' cadastrado(a) com sucesso!\nE agora acessível na lista de professores.", nome, sbNome),
